# Guide Library

This folder is the growing guide library for `howtouseai`.

The goal is to turn the repository into a clear, beginner-friendly knowledge base for:

- AI coding
- AI agents
- prompting and workflow
- safe Git and debugging habits
- self-hosting and deployment
- infrastructure basics
- RAG and retrieval workflows
- embeddings and vector search
- recommendation systems
- stack selection and practical tradeoffs

## Current Tracks

### Project Context

- [Why This Exists](./why-this-exists.md)

### Getting Started

- [How to Start with AI Coding](./how-to-start-with-ai-coding.md)
- [Path 1: I Have a Windows Laptop or PC](./path-1-windows-pc.md)
- [Path 2: I Have a Mac](./path-2-mac.md)
- [Path 3: I Have Linux](./path-3-linux.md)
- [Path 4: My Computer Is Weak, or I Want a Remote Setup](./path-4-vps-remote-setup.md)
- [Path 5: I Only Have a Phone](./path-5-phone-only.md)
- [First Project: Start Small and Finish Something](./first-project.md)
- [How to Choose a Stack](./how-to-choose-a-stack.md)
- [Frontend vs Backend](./frontend-vs-backend.md)

### Architecture and Backend

- [API Basics and API Stacks](./api-basics-and-api-stacks.md)

### Prompting and Agent Workflow

- [Prompting Basics: How to Ask Your AI Coding Agent Correctly](./prompting-basics.md)
- [Workflow Rules: How to Work With an AI Coding Agent Without Creating Chaos](./workflow-rules.md)
- [AI Agent Rules: How Your Coding Agent Should Behave Inside Your Repository](./ai-agent-rules.md)
- [MCP Basics](./mcp-basics.md)

### Safe Development

- [Git Safety: How to Avoid Losing Work](./git-safety.md)
- [Troubleshooting: What to Do When Something Breaks](./troubleshooting.md)

### Self-Hosting and Deployment

- [How to Get a Domain and Connect It Through Cloudflare](./how-to-get-domain.md)
- [Deploy Basics: Real Self-Hosting From Your Own Server](./deploy-basics.md)
- [Useful Tools for AI: What to Install on a Server So an AI Agent Can Work Properly](./useful-tools-for-ai.md)

## Planned Tracks

The repository is also growing toward guides such as:

- RAG basics
- how embeddings work and how to use them
- vector search and retrieval
- recommendation systems
- how AI stacks differ
- local vs VPS vs managed deployment tradeoffs
- practical self-deployment playbooks

## Recommended Reading Order

If you are brand new:

1. [How to Start with AI Coding](./how-to-start-with-ai-coding.md)
2. One of the setup paths (`Path 1` to `Path 5`)
3. [First Project: Start Small and Finish Something](./first-project.md)
4. [How to Choose a Stack](./how-to-choose-a-stack.md)
5. [Frontend vs Backend](./frontend-vs-backend.md)
6. [API Basics and API Stacks](./api-basics-and-api-stacks.md)
7. [Prompting Basics: How to Ask Your AI Coding Agent Correctly](./prompting-basics.md)
8. [Workflow Rules: How to Work With an AI Coding Agent Without Creating Chaos](./workflow-rules.md)
9. [Git Safety: How to Avoid Losing Work](./git-safety.md)
10. [Troubleshooting: What to Do When Something Breaks](./troubleshooting.md)

If you want self-hosting next:

1. [How to Get a Domain and Connect It Through Cloudflare](./how-to-get-domain.md)
2. [Deploy Basics: Real Self-Hosting From Your Own Server](./deploy-basics.md)
3. [Useful Tools for AI: What to Install on a Server So an AI Agent Can Work Properly](./useful-tools-for-ai.md)
